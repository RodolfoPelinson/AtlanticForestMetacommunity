...
var_partitioning <- function(Y, env, clim = NULL, spa, percent_r2 = F, allow_negative_r2 = TRUE){
...
##################################################
  ...
  if(is.null(clim) == FALSE & (is.null(spa) || nrow(spa) == 0)){
    comp.varpart <- varpart(Y, env, clim)

    independent.variables <- data.frame(env,clim)
    #spa_clim <- data.frame(clim,spa)
    env_clim <- data.frame(clim,env)
    #env_spa <- data.frame(spa,env)

    anova_pure_env <- anova.cca(rda(Y, env, clim),permutations=9999)
    #anova_pure_spa <- anova.cca(rda(Y, spa, env_clim),permutations=9999)
    anova_pure_clim <- anova.cca(rda(Y, clim, env),permutations=9999)
    ...
    r2_all <- comp.varpart$part$fract$Adj.R.square[3]
    r2_pure_env <- comp.varpart$part$indfract$Adj.R.square[1]
    r2_pure_clim <- comp.varpart$part$indfract$Adj.R.square[3]
    r2_pure_spa <- NA
    r2_share_env_spa <- NA
    r2_share_clim_spa <- NA
    r2_share_clim_env <- comp.varpart$part$indfract$Adj.R.square[2]
    r2_share_clim_env_spa <- NA
    r2_resid <- comp.varpart$part$indfract$Adj.R.square[4]
