...
```{r, fig.height= 1.5,fig.width=3,out.width= 400, out.height=200 , fig.align='center', dpi = 200}
...
```{r, fig.height= 1.5,fig.width=1.5,out.width= 200, out.height=200 , fig.align='center', dpi = 200}
par(mfrow = c(1,1))
...
#Testing significande of spatially structured environment + climate
Broad_MEM_env_clim <- listw.select(data.frame(Broad_env_FS, Broad_clim_FS), candidates = candidates_Broad_env, MEM.autocor = c("positive"), method = c("FWD"), MEM.all = FALSE, nperm = 10000, nperm.global = 10000, alpha = 0.05, p.adjust = FALSE, verbose = FALSE)

Broad_env_clim_spa_sig <- envspace.test(spe = Broad_pa, env = data.frame(Broad_env_FS, Broad_clim_FS),   coord = Broad_coord, MEM.spe =  data.frame(Broad_MEM$best$MEM.select), 
              listw.env = candidates_Broad_env[[Broad_MEM_env_clim$best.id]], MEM.autocor = c("positive"),
              regular = FALSE, nperm = 10000, MSR.method = "singleton", alpha = 0.05)
```
\  
\  
...
NI_env_spa_sig <- envspace.test(spe = NI_pa, env = NI_env_FS,   coord = NI_coord,  
              MEM.spe =  NI_MEM_FS, 
              listw.env = candidates_NI_env[[NI_MEM_env$best.id]], MEM.autocor = c("positive"),
              regular = FALSE, nperm = 10000, MSR.method = "singleton", alpha = 0.05)
